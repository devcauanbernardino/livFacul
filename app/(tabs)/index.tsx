  ...
  return (
    <SafeAreaView style={estilos.telaSegura}>
      ...
      <View style={estilos.cabecalho}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={estilos.textoBemVindo}>Olá, {usuario?.nome || "Visitante"}</Text>
          <Text style={estilos.textoTitulo}>Explorar eBooks 📚</Text>
        </View>

        <View style={estilos.caixaAvatar}>
          <Avatar localUri={usuario?.avatarLocal?.uri} remoteUrl={usuario?.avatarUrlRemota || null} size={40} />
        </View>
      </View>
            ...
            {termoBusca.length === 0 && (
              ...
              <View style={estilos.bannerBox}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={estilos.containerCategorias}>
                  {["Ficção", "Romance", "Tecnologia", "Educação", "Autoajuda", "Negócios", "Fantasia", "Biografia"].map(
                    (categoria) => (
                      <TouchableOpacity 
                        key={categoria} 
                ...
                <Text style={{ color: COLORS.text, fontSize: 18, fontWeight: 'bold', marginLeft: 20, marginBottom: 10 }}>
                  Recentes
                </Text>
              </>
            )}
            {termoBusca.length > 0 && (
               <Text style={{ color: COLORS.sub, fontSize: 14, marginLeft: 20, marginBottom: 10 }}>
          ...
          !carregando ? (
            <View style={{ alignItems: 'center', marginTop: 40 }}>
              <Text style={{ color: COLORS.sub }}>
                {termoBusca ? "Nenhum livro encontrado com esse termo." : "Nenhum livro cadastrado."}
              </Text>
            </View>
          ) : (
            ...
            >
              <View style={estilos.cartaoLivro}>
                ...
                <View style={estilos.infoContainer}>
                  {categoriaFinal && (
                    <Text style={estilos.categoriaLivro}>{categoriaFinal.toUpperCase()}</Text>
                  )}
                  ...
                  <View style={estilos.linhaPreco}>
                    ...
                    {item.condicao && (
                      <View style={[
                          estilos.tagCondicao, 
                          { backgroundColor: item.condicao === 'novo' ? '#4ADE8020' : '#FACC1520' }
                      ]}>
                          <Text style={[
                              estilos.textoCondicao,
                              { color: item.condicao === 'novo' ? '#4ADE80' : '#FACC15' }
                          ]}>
...
const estilos = StyleSheet.create({
  ...
  telaSegura: { flex: 1, backgroundColor: COLORS.bg, paddingTop: StatusBar.currentHeight || 0 },
  menuLateral: { position: "absolute", top: 0, bottom: 0, width: 260, backgroundColor: COLORS.bg, borderRightWidth: 1, borderRightColor: "#2A2735", zIndex: 20, paddingHorizontal: 16, paddingTop: 20 },
  fundoEscuro: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(0,0,0,0.5)", zIndex: 10 },
  cabecalho: { flexDirection: "row", alignItems: "center", paddingHorizontal: 20, paddingBottom: 10, paddingTop: 8 },
  botaoMenu: { width: 36, height: 36, borderRadius: 10, borderWidth: 1, borderColor: COLORS.inputBorder, backgroundColor: COLORS.inputBg, alignItems: "center", justifyContent: "center", paddingVertical: 6, paddingHorizontal: 8, marginRight: 14 },
  ...
  campoBusca: { backgroundColor: COLORS.inputBg, borderColor: COLORS.inputBorder, borderWidth: 1, borderRadius: 12, color: COLORS.text, paddingHorizontal: 14, paddingVertical: 10, fontSize: 15 },
  bannerWrapper: { alignSelf: "center", marginBottom: 20, borderRadius: 16, overflow: "hidden", backgroundColor: COLORS.inputBg, borderWidth: 1, borderColor: COLORS.inputBorder, marginTop: 10 },
  bannerBox: { flex: 1, justifyContent: "flex-end", paddingVertical: 10, paddingHorizontal: 12, backgroundColor: "rgba(0,0,0,0.35)" },
  bannerTitulo: { color: "#fff", fontSize: 16, fontWeight: "700" },
  bannerSub: { color: "#d1d5ff", fontSize: 12, marginTop: 2 },
  containerCategorias: { paddingHorizontal: 20, paddingVertical: 8, marginBottom: 20 },
  etiquetaCategoria: { height: 40, minWidth: 96, paddingHorizontal: 16, borderRadius: 20, backgroundColor: COLORS.inputBg, borderColor: COLORS.accent, borderWidth: 1, marginRight: 10, alignItems: "center", justifyContent: "center" },
  textoCategoria: { color: COLORS.accent, fontSize: 14, fontWeight: "600" },
  
  cartaoLivro: { flexDirection: "row", backgroundColor: COLORS.inputBg, borderColor: COLORS.inputBorder, borderWidth: 1, borderRadius: 16, padding: 12, marginBottom: 16, marginHorizontal: 20 },
  capaContainer: { width: 80, height: 110, borderRadius: 8, overflow: 'hidden', backgroundColor: "#2A2735", marginRight: 12 },
  ...
  categoriaLivro: { color: COLORS.accent, fontSize: 10, fontWeight: "700", marginBottom: 2 },
  ...
  botaoVender: { position: "absolute", right: 20, bottom: 28, flexDirection: "row", backgroundColor: COLORS.button, paddingVertical: 12, paddingHorizontal: 16, borderRadius: 14, alignItems: "center", elevation: 6, shadowColor: COLORS.button, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 4 },
  textoMais: { color: "white", fontSize: 20, fontWeight: "700", marginRight: 6, lineHeight: 22 },
